...
export class LocalPlugins extends PluginsAbstract<LocalEnvironment> {
    public listSources(_filters?: List<IRelateFilter> | IRelateFilter[]): Promise<List<IPluginSource>> {
        throw new NotSupportedError(`${LocalPlugins.name} does not support listing plugin sources`);
    }

    public addSources(_urls: string[]): Promise<IPluginSource> {
        throw new NotSupportedError(`${LocalPlugins.name} does not support adding plugin sources`);
    }

    public removeSources(_urls: string[]): Promise<IPluginSource> {
        throw new NotSupportedError(`${LocalPlugins.name} does not support removing plugin sources`);
    }
